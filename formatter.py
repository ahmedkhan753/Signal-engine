def format_cli_output(response):
    ...
    for facet in response.get("facets", []):
        ...
        # Optionally, we could add facet-level traces here if requested,
        # but the prompt example only shows Score, Status, Summary.
        # We will keep point deductions inside facet-level trace if we display them,
        # but the prompt said "Keep point deductions visible inside facet-level traces for transparency/debugging."
        # We can add a "Trace:" under each facet.
        if facet.get("trace"):
